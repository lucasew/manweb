...
from utils_error import report_error
...
for line in f:
    try:
        ...
        report_error(e, context=f"Failed parsing line: {line.strip()}")
...
for drv_hash, v in items_by_hash.items():
    ...
    if not folder_with_hash.exists(): # if the hash was not looked yet, download the nar and get the stuff
        try:
            narinfo = urlopen(f"https://cache.nixos.org/{drv_hash}.narinfo", timeout=10)
        except Exception as e:
            report_error(e, context=f"Failed to fetch narinfo for {drv_hash}")
            continue
        ...
        with TemporaryDirectory() as tempdir_name:
            ...
            try:
                res = urlopen(nar_url, timeout=30)
            except Exception as e:
                report_error(e, context=f"Failed to fetch {nar_url}")
                continue
            ...
            try:
                ...
                report_error(e, context=f"Failed to extract or process {nar_url}")
                continue
    ops.update(len(v))
